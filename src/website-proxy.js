...
const host = (base) => { try { return new URL(base).host; } catch { return base; } };
  ...
  try {
    if (path === "/health" || path === "/api/demo/status") {
      ...
      const services = Object.fromEntries(await Promise.all(Object.entries(urls).map(async ([name, base]) => {
        try { const result = await fetch(base + (name === "broker" ? "/health" : "/api/health"), { signal: AbortSignal.timeout(8000), cache: "no-store" }); return [name, { ok: result.ok, status: result.status, url: base }]; }
        catch { return [name, { ok: false, status: 0, url: base }]; }
      })));
    ...
    if (path.startsWith("/v1/")) {
      ...
      return relay(await fetch(urls.broker + path + url.search, { method: request.method, headers, body: request.method === "POST" ? JSON.stringify(body ?? {}) : undefined, signal: AbortSignal.timeout(12000), cache: "no-store" }), `${url.host} -> ${host(urls.broker)}`);
    }
    if (path === "/demo/sessions" && request.method === "POST") {
      return relay(await fetch(urls.broker + path, { method: "POST", headers: { "content-type": "application/json" }, body: "{}", signal: AbortSignal.timeout(12000) }), `${url.host} -> ${host(urls.broker)}`);
    }
    const match = /^\/demo\/sessions\/([a-f0-9-]{36})(\/.*)?$/.exec(path);
    if (!match) return json({ error: { code: "NOT_FOUND", message: "Demo route not found." } }, 404);
    if (!match[2] && request.method === "DELETE") return relay(await fetch(urls.broker + path, { method: "DELETE", signal: AbortSignal.timeout(12000) }), `${url.host} -> ${host(urls.broker)}`);
    ...
    }), `${url.host} -> ${host(service)} (${receiver ? "receiver" : "sender"}) -> ${host(urls.broker)}`);
  } catch {
    return json({ error: { code: "DEMO_UNAVAILABLE", message: "The live services could not complete this request. Please try again." } }, 502);
  }
}

async function relay(response, via) {
  try { return json(await response.json(), response.status, via ? { "x-demo-via": via } : {}); }
