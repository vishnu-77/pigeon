...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { live: false, error: "The demo request could not be processed." },
      { status: 400 }
    );
  }
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          error:
            response.status >= 500
              ...
              : "The demo request could not be processed."
        },
    ...
    const ciphertext =
      input.scenario === "message"
        ? payload?.receiver?.delivered?.[0]?.data?.message
        : undefined;
    ...
    return NextResponse.json({
      ...
      gates: Array.isArray(payload.gates)
        ? payload.gates.map((gate: { name?: string; pass?: boolean }) => ({
            name: gate.name,
      ...
      receiver: {
        ...
        delivered: ciphertext ? [{ data: { message: ciphertext } }] : []
      }
    });
  } catch {
    return NextResponse.json(
