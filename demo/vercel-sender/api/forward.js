export default async function handler(req, res) {
  ...
  if (!/^[a-f0-9-]{36}$/.test(sessionId ?? "") || !allowed || !["checkout", "catalog"].includes(principal) || JSON.stringify(req.body).length > 8192) {
    return res.status(400).json({ error: { code: "BAD_REQUEST", message: "Invalid sender operation." } });
  }
  try {
    const headers = { "content-type": "application/json", "x-pigeon-region": "uk", authorization: `Bearer ${principal === "catalog" ? "catalog-token" : "checkout-token"}` };
