export default async function handler(req, res) {
  ...
  const allowed = (method === "POST" && (path === "/v1/contracts" || path === "/v1/subjects/payments.authorize/receive" || /^\/v1\/subjects\/payments\.authorize\/messages\/[a-z0-9_-]+\/ack$/.test(path))) ||
  ...
  try {
    const headers = { "content-type": "application/json", "x-pigeon-region": "uk", authorization: "Bearer gateway-token" };
