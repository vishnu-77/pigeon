export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(result.ok ? 200 : 503).json({ ok: result.ok, service: "pigeon-receiver", broker: result.status });
  } catch { return res.status(503).json({ ok: false, service: "pigeon-receiver", error: "Broker unavailable" }); }
}
