...
test("serves the PigeonMQ landing page and its assets", async () => {
  const response = await fetch(`${base}/`);
  assert.equal(response.status, 200);
  assert.match(await response.text(), /Messaging with/);
  for (const path of ["/assets/landing.css", "/assets/landing.js", "/assets/pigeon-mark.svg"]) {
